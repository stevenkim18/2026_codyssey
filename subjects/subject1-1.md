...
- 확인 방법(예시)
  ...
  - 포트 리슨 상태 확인: `ss -tulnp` 실행 후 sshd 관련 라인 확인
...
- 그룹: `agent-core`
- 권한: `750` (`rwxr-x---`)
- cron 실행 계정: `agent-admin` (`agent-admin`은 `agent-core`에 포함되어 실행 가능해야 함)
...
- `monitor.log`가 커지면 최대 10MB/10개 파일을 유지한다.
- 구현 방법은 자유다. (`logrotate` 사용 또는 스크립트 로직 구현)

### 5. 자동 실행(cron) 설정

- `agent-admin` 계정의 crontab으로 `monitor.sh`를 매분 실행되도록 등록한다.
- 등록 후 1~2분 내 `monitor.log`에 새 라인이 자동으로 누적되는 것을 확인한다.

## 5. 보너스 과제 (선택)

### 보너스 1 - `report.sh`로 요약 리포트 자동 생성

- `monitor.log`를 분석해 CPU/MEM/DISK의 평균/최대/최소와 샘플 수를 콘솔로 출력한다.
- 선택 사항으로 시작/종료 시간을 입력받아 해당 구간의 로그만 분석한다.
...
- 디렉토리 미존재, 권한 부족, 대상 파일 0개 등에서 안전하게 종료하거나 경고하도록 처리한다.
...
- 필요한 경우에만 sudo를 사용한다. 가능한 일반 계정으로 진행한다.
